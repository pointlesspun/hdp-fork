...
This fork contains adaptations to build WinUtils which is needed to run Apache Spark on Windows. 
...
In order to build WinUtils we'll generally follow the steps in Building.txt, however we will be making some changes and have different requirements.
...
Basically you'll need to go to this file and change the paths to match the setup you have on your computer.


We use vcpkg (https://github.com/microsoft/vcpkg.git) for installing Boost, Protocol buffers,
OpenSSL and Zlib dependencies. Run the following commands to set up these dependencies.
...
If you only need WinUtils however you can pick up the hadoop-common-3.5.0 tar which contains the winutils executables from `C:\hdp\hadoop-common-project\hadoop-common\target`.
